    ...
    override fun onCreateView(
        ...
        btnPedirCita.setOnClickListener {
            if (horaSeleccionada != null) {
                ...
                // Por ejemplo, agregarla al com.example.uf1_proyecto.objects.RecordatorioManager o pasarla como un argumento
                ...
                if (fechaSeleccionada.get(Calendar.DAY_OF_YEAR) == fechaActual.get(Calendar.DAY_OF_YEAR)) { //Si la fecha selerccioanda es la misma que la de dia de hoy
                    if (horaSeleccionadaNumerica < fechaActual.get(Calendar.HOUR_OF_DAY) ||
                        (horaSeleccionadaNumerica == fechaActual.get(Calendar.HOUR_OF_DAY) && minutosSeleccionados < fechaActual.get(
                            Calendar.MINUTE
                        ))) {
                        Toast.makeText(
                            context,
                ...
                if (fechaSeleccionada.timeInMillis < fechaActual.timeInMillis) {
                    Toast.makeText(context, "Ingrese una fecha válida.", Toast.LENGTH_SHORT).show()
